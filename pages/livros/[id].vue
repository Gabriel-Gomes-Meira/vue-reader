<template>
  <div class="container mx-auto grid
    ...
    <div v-for="(data) in livros" :key="data.id"
      ...
      <NuxtLink :to="`/reader/${data.id}`">
        <div class="card-body">
          ...
          <h2 class="card-title">
            <span class="truncate md:text-clip md:overflow-visible md:whitespace-normal"> {{ data.nome }} </span>
          </h2>
        </div>
      </NuxtLink>
    </div>
...
const carregarLivros = async (id) => {
  try {
    const response = await fetch(`/api/livros?serieId=${id}`)
